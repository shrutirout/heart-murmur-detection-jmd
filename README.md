...
The following folder is not included in the repository due to its size and is generated by running the pipeline:

- `data/decomposed/` (approximately 6 GB) -- created by decompose_parallel.py

The preprocessed signal files in `data/processed/` are included in the repository. No folders need to be created manually.
...
2. Run notebooks 01 through 04 in order to explore the data, preprocess the signals, implement JMD, and select parameters. Note: the preprocessed signal files are already included in the repository so notebook 02 does not need to be rerun unless you want to redo preprocessing from scratch.
...
If you only want to run inference using the pre-trained model and skip all training, go directly to notebook 21 after placing decomposed .npz files in `data/decomposed/unknown/`. The preprocessed .npy files are already included in the repository.
